...
enum resolution {
  ...
  return (
      ...
      <div className=' fixed w-full top-0 xl:my-6 my-2  flex justify-center '>
    <nav className={`shadow-md  ${isOpen ? 'sm:w-full' : 'w-3/4' } flex justify-between items-center dark:bg-gray-900 bg-gray-100 px-4 py-2 rounded-3xl transition-all duration-600 ease-in`} >
      ...
      {/*HAMBURGER ICON HERE*/}
      <div onClick={toggleNavbar} className="w-6 h-6 cursor-pointer md:hidden  transition-all duration-500 ease-in ">
        ...
        ${isOpen ? 'top-14 rounded-lg h-screen bg-gray-300 ' : 'top-[-490px]'} `}>
          {navLinks.map((link) => (
            <li key={link.name} className='my-7 md:my-0'>
              <Link
                href={link.href}
                className={`px-3 py-2 rounded-md text-sm font-medium text-gray-500 hover:text-gray-200 dark:text-gray-500 hover:text-gray-800 dark:hover:text-gray-100  transition-all duration-500 ease-in`}
              >
                {link.name}
              </Link>
            </li>
